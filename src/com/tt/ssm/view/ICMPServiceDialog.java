...
public class ICMPServiceDialog extends JDialog implements ActionListener {
	...
	public ICMPServiceDialog(JFrame parent, Callback callback) {
		super(parent);
		this.callback = callback;
		setTitle("ICMP Service");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ("test".equals(e.getActionCommand())) {
			final ICMPService service = getService();
			if (service != null) {
				EventQueue.invokeLater(new Runnable() {
					@Override
					public void run() {
						service.request();
						JOptionPane.showMessageDialog(ICMPServiceDialog.this, service.getResponse(), "Test", JOptionPane.INFORMATION_MESSAGE);
					}
				});
			}
		}
		if ("ok".equals(e.getActionCommand())) {
			ICMPService service = getService();
			if (service != null) {
				callback.onICMPServiceAdded(service);
				dispose();
			}
		}
	}
	...
	public interface Callback {
		...
		public void onICMPServiceAdded(ICMPService service);
		
	}
	
	/* ********** private ********** */
	
	private ICMPService getService() {
		// name
		...
		return (new ICMPService(name, group, host, interval, warning, error));
	}
	...
	private JPanel createContentPanel() {
		...
		sl.putConstraint(SpringLayout.NORTH, intervalLabel, 10, SpringLayout.SOUTH, host);
		
		sl.putConstraint(SpringLayout.EAST, interval, -10, SpringLayout.EAST, panel);
		sl.putConstraint(SpringLayout.NORTH, interval, 10, SpringLayout.SOUTH, host);
